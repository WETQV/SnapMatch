...
1. Проверить, что в проекте нет приватных токенов, базы сообщений, пользовательских загрузок и локальных моделей.
2. Проверить `README.md`, `docs/USAGE.md`, `docs/BUILD_AND_RELEASE.md`.
3. Выполнить `build.bat`.
4. Запустить `dist\SnapMatch.exe` и проверить старт GUI.
5. Выполнить `build_installer.bat`.
6. Создать GitHub Release `v1.0.4`.
7. Приложить `SnapMatch-Setup-1.0.4.0.exe`.
8. В release notes указать, что Linux build пока не подтверждён.
